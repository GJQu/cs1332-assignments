...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new java.lang.IllegalArgumentException("key or value cannot be null");
        }
        double newLoadFactor = (size + 1.0) / table.length;
    ...
    private void resizeBackingTable(int length) {
        ...
        for (int i = 0; i < table.length; i++) {
            ...
            while (current != null) {
                ...
                int newIndex = Math.abs(key.hashCode() % newTable.length);
                // insert entry at the fornt of the chain in the new table
                newTable[newIndex] = 
                    new ExternalChainingMapEntry<>(key, value, newTable[newIndex]);
                current = current.getNext();
            }
        }
        // replace old table iwth new one
        table = newTable;
    }
